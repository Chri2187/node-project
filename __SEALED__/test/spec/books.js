...
describe('/books', () => {
    afterEach(async () => {
        ...
        if (
            collections.some(
                (collection) => collection.collectionName === 'books'
            )
        ) {
    ...
    it('POST responds with 200 on correct body', async () => {
        const response = await request(
            `http://server:${process.env.SERVER_PORT}`
        )
            .post('/books')
    ...
    describe('/[id]', () => {
        ...
        it('GET responds with a POST inserted document', async () => {
            ...
            const post_response = await request(
                `http://server:${process.env.SERVER_PORT}`
            )
                .post('/books')
                .set('Accept', 'application/json')
            ...
            const get_response = await request(
                ...
                .expect('Content-Type', /json/)
                .expect(200);
            const { _id, ...response_book } = get_response.body;
